...
8. Run the [Central Asia microelectronics cookbook](cookbooks/secondary-sanctions/README.md) to exercise the native `MemoArtifact -> Agenda Intelligence -> versioned review bundle` seam offline in about one minute.
9. Run the [two-case portfolio cookbook](cookbooks/flagship-portfolio/README.md) to atomically build two source-backed review bundles and deterministic ZIP distributions.
10. Run `python scripts/run_verification_benchmark.py` for two golden variants, four controlled failures, and a SARIF 2.1.0 regression receipt.
