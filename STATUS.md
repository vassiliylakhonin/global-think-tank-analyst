...
Updated 2026-09-01. Maturity definitions are in
...
| Method evidence | M3 | Four disclosed 12-case Antigravity runs: two Gemini and two Claude. All report 12/12 contract passes with the skill vs. 0/12 for the baseline. The first Claude run stores 181 capped skill warnings under its original ruleset and 170 under the 1.2.2 rescore; the fresh [post-change replication](evals/agent-eval/runs/2026-09-01-antigravity-claude-opus-4.6-thinking-seed-20260902/) stores 111, with three samples at the cap. Exact artifacts, freshness comparisons, triage, and limitations are published. | Add explicit truncation telemetry, structured-artifact evaluation, and broader task domains; practitioner validation remains tracked under U |
...
2. Expose per-sample warning truncation explicitly in JSON reports instead of
   relying on prose caveats around the 25-finding cap.
3. Add a structured `MemoArtifact` evaluation path and version a broader task
   suite without rewriting the frozen 12-case benchmark.
