...
**Status:** Literature-reported. Reproduction attempted 2026-07-06: **0/2 fresh-context runs** (Claude Opus 4.8, per-section confidence with explicit justification in both). Retained as precautionary; probe may under-elicit the accuracy failure the literature describes. See [`self-runs/2026-07-06-fm-repro.md`](self-runs/2026-07-06-fm-repro.md).
...
**Status:** Literature-reported. Reproduction attempted 2026-07-06: **0/3 fresh-context runs** (Claude Opus 4.8, incl. one control without author-global settings; all runs tagged the counterclaim `[user-provided]` and held direction). Retained as precautionary — the literature uses up to 5 pushback turns and other models. See [`self-runs/2026-07-06-fm-repro.md`](self-runs/2026-07-06-fm-repro.md).

---

## 25. Silent input-claim drop

**Symptom:** A memo built on a user-provided source packet or an ingested key-claims table uses some input claims and simply ignores others — no flag, no conflict note, no out-of-scope statement. The reader cannot tell selection from omission.
**Cue:** Compare the memo against the input claim list: a claim is neither referenced nor is its absence accounted for anywhere. Drop bait is typically the inconvenient claim (contradicts the narrative), the tangential one, or the unverifiable one.
**Why it matters:** Per-claim provenance tags audit only the claims that made it into the output; three-value logic covers only the uncertain claims the analyst engaged with. Neither makes a dropped input claim visible — and the dropped claim is often exactly the one that would have changed the judgment.
**Fix:** Per the Input-claim accounting rule in [`../AGENTS.md`](../AGENTS.md): every input claim ends as used, flagged-but-not-used, conflict-surfaced, or out-of-scope with a reason; a short accounting line near the limitation note suffices.
**Status:** Canon-motivated (adopted 2026-07-10 with the Input-claim accounting rule). Reproduction attempted 2026-07-10: **0/2 fresh-context runs** without the rule (Claude Opus 4.8, 8-claim table probe with five drop-bait claims — all accounted for anyway). With the rule loaded, 2/2 runs additionally produced an explicit accounting block, which no rule-free run did. Retained as precautionary; the rule's verified effect on this probe is making coverage auditable, not preventing an observed drop. Not promoted to `SKILL.md` self-check (reproduction gate not met). Harder probes for a future attempt: claims embedded in prose, 15+ claims, multi-source packets. See [`self-runs/2026-07-10-input-claim-accounting-repro.md`](self-runs/2026-07-10-input-claim-accounting-repro.md).
